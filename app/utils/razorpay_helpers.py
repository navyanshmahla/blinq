...
from app.config import razorpay_config

client = razorpay.Client(auth=(razorpay_config.KEY_ID, razorpay_config.KEY_SECRET))

def create_or_get_razorpay_customer(user: User) -> str:
    """Get existing Razorpay customer ID or create new one"""
...
def handle_pro_expiration(db: Session, user: User) -> bool:
    ...
    if user.subscription_tier == 'pro' and user.pro_expires_at:
        if user.pro_expires_at < datetime.utcnow() and not user.razorpay_subscription_id:
